...
for (const target of targets) {
  ...
  const outfile = `dist/monq-${target.os}-${target.arch}`
...
// If single target, also create a simple "monq" copy
if (singleFlag && targets.length === 1) {
  const target = targets[0]!
  const source = `dist/monq-${target.os}-${target.arch}`
  const dest = "dist/monq"

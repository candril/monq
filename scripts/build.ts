...
if (olderThan(Bun.version, BUN_FLOOR)) {
  console.error(`monq needs bun ${BUN_FLOOR} or newer to build; this is ${Bun.version}`)
  process.exit(1)
}
...
// `dist/monq`; CI and `--all` builds carry the platform in the name. (Copying the
...
for (const target of targets) {
  const outfile = localBuild ? "dist/monq" : `dist/monq-${target.os}-${target.arch}`
  console.log(`monq ${version} → ${outfile}`)

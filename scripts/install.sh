...
#
#   MONQ_VERSION=0.2.0              install a specific release instead of the latest
#   MONQ_INSTALL_DIR=~/.local/bin   install somewhere other than /usr/local/bin
#
# Homebrew and Nix users have shorter routes:
#   brew install candril/tap/monq
#   nix run github:candril/monq
#
...
if ! command -v monq >/dev/null 2>&1; then

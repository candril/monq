...
export function BulkEditConfirmDialog({ confirmation, focusedIndex }: BulkEditConfirmDialogProps) {
  ...
  if (missingCount > 0) {
    ...
    for (const doc of missing) lines.push({ text: `  ${docSummary(doc as Record<string, unknown>)}`, danger: true })
  }
  if (addedCount > 0) {
    if (missingCount > 0) lines.push({ text: "" })
    lines.push({ text: `${addedCount} new doc${addedCount === 1 ? "" : "s"} added to array:`, dim: true })
    for (const doc of added) lines.push({ text: `  ${docSummary(doc as Record<string, unknown>)}` })
  }

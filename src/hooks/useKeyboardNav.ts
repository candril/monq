        ...
        case "f": {
          ...
          if (state.pipelineMode) {
            ...
            const filterTokens = existingTokens.filter(
              (t: string) =>
                !t.startsWith("+") && !(t.startsWith("-") && !/[><!:]/.test(t.slice(1))),
            )
        ...
        case "e": {
          ...
          openEditorForMany(
            ...
            .then(async (outcome) => {
              ...
              const showConfirm = (cr: typeof result, ce: typeof editedDocs) =>
                dispatch({
                  ...
                  confirmation: {
                      ...
                      openEditorForMany(
                        ...
                        .then(async (o2) => {
                          ...
                          if (o2.result.missing.length === 0 && o2.result.added.length === 0) {
                            ...
                            showConfirm(o2.result, o2.editedDocs)
                          }
                        })
                        .catch((err: Error) => {
                          renderer.resume()
                    ...
                    resolve: async (missingAction, addedAction) => {
                      ...
                      if (errors.length > 0) {
                        ...
                        if (missingAction === "delete" && cr.missing.length > 0)
                          parts.push(`${cr.missing.length} deleted`)
                        if (addedAction === "insert" && cr.added.length > 0)
                          parts.push(`${cr.added.length} inserted`)
                        dispatch({
                          type: "SHOW_MESSAGE",
